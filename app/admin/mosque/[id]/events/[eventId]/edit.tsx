...
export default function EditEventScreen() {
  ...
  const { data: events } = useMosqueEvents(id || "", true);
  ...
  useEffect(() => {
    if (currentEvent) {
      setFormData({
        ...
        scholar: currentEvent.scholar || "",
        event_date: currentEvent.event_date || "",
        event_time: currentEvent.event_time || "",
      });

      // Set initial date and time for pickers
      if (currentEvent.event_date) {
        setSelectedDate(new Date(currentEvent.event_date));
      }
      if (currentEvent.event_time) {
        const [hours, minutes] = currentEvent.event_time.split(":");
    ...
    <View className="flex-1 bg-gray-50">
      ...
      <ScrollView className="flex-1 p-4">
        ...
        <TouchableOpacity
          ...
          disabled={isPending}
          className="w-full py-4 bg-blue-600 rounded-lg mb-8"
        >
          {isPending ? (
